...
export default function TeamActions({ memberId, onDelete }: TeamActionsProps) {
    ...
    return (
        <div className="relative" ref={menuRef}>
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-card rounded-md shadow-lg border border-border z-10 py-1 animate-in fade-in zoom-in duration-100">
                    <button
                        onClick={() => {
                            setIsOpen(false);
                            alert('Edit details feature requires backend integration.');
                        }}
                    ...
                    <button
                        onClick={() => {
                            setIsOpen(false);
                            alert('Change Role feature coming soon');
                        }}
                        className="flex w-full items-center px-4 py-2 text-sm text-foreground hover:bg-muted transition-colors"
                    >
                        ...
                        onClick={async () => {
                            ...
                            if (confirm('Are you sure you want to remove this team member?')) {
                                try {
                                    ...
                                    // Call parent onDelete to update UI if needed, or refresh page
                                    onDelete();
